...
class Maplibre {
    ...
    init(Conf) {
        return new Promise((resolve, reject) => {
            ...
            this.map = new maplibregl.Map({
                ...
                antialias: true, hash: true, maxBounds: Conf.map.maxBounds, center: Conf.map.viewCenter, pitch: Conf.map.viewPitch
            });
    ...
    getZoom(round) {
        return round ? Math.round(this.map.getZoom()) : this.map.getZoom();
    };
    ...
    addLine(data, target) {
        console.log("geolib: addline: " + target);
